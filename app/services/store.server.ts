...
export async function ensureStore(
  shopDomain: string,
  opts: { scopes: string; apiVersion: string },
): Promise<Store> {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const store = await tx.store.create({
      data: { organizationId: org.id, shopDomain: domain },
    });
    ...
    await tx.taxConfig.create({
      data: { storeId: store.id, treatment: "UNCONFIGURED" },
    });
    await tx.notificationPreference.create({ data: { storeId: store.id } });
    await tx.subscription.create({
      data: { storeId: store.id, planKey: "free" },
    });
    return store;
  });
}
...
export async function markUninstalled(shopDomain: string): Promise<void> {
  ...
  await prisma.$transaction([
    ...
    prisma.installation.updateMany({
      where: { storeId: store.id, uninstalledAt: null },
    ...
    prisma.adAccount.updateMany({
      where: { storeId: store.id },
      data: { credentialsEnc: null, status: "DISCONNECTED" },
    }),
