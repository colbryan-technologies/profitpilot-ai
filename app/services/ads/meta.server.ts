...
async function call<T>(
  ...
  if (!res.ok) {
    ...
    throw new AdsProviderError(
      `Meta API error ${res.status}: ${err?.message ?? "unknown"}`,
      res.status === 429 ||
        res.status >= 500 ||
        err?.code === 17 ||
        err?.code === 4,
...
export const metaAds: AdsProvider = {
  ...
  async listAccounts(creds) {
    ...
    let url: string | undefined =
      `${GRAPH}/me/adaccounts?fields=id,name,currency,account_id&limit=100&access_token=${encodeURIComponent(creds.accessToken)}`;
    while (url) {
      const page: z.infer<typeof accountsSchema> = await call(
        url,
        accountsSchema,
      );
      out.push(
  ...
  async fetchDailySpend(creds, externalId, start, end) {
    ...
    while (url) {
      const page: z.infer<typeof insightsSchema> = await call(
        url,
        insightsSchema,
      );
      for (const row of page.data) {
        ...
        out.push({
          ...
          attributedRevenueMinor: purchaseValue
            ? toMinor(purchaseValue, currency)
            : null,
          attributedConversions: purchases
            ? Math.round(Number(purchases))
            : null,
        });
      }
      url = page.paging?.next;
