...
export async function dailySeries(
  ...
  await requireReportPeriod(storeId, period);
  const rows = await prisma.profitSnapshot.findMany({
    where: { storeId, date: { gte: period.start, lt: period.end } },
    orderBy: { date: "asc" },
  });
  return rows.map((r) => ({
...
export async function orderProfitsInPeriod(
  storeId: string,
  period: Period,
...
> {
  ...
  const [store, calc, rows] = await Promise.all([
    ...
    prisma.order.findMany({
      where: {
        storeId,
        processedAt: {
          gte: new Date(period.start.getTime() - 86_400_000),
          lt: new Date(period.end.getTime() + 86_400_000),
        },
      },
  ...
  return rows
    ...
    .map((r) => ({
      profit: computeOrderProfit(toOrderInput(r), calc),
      name: r.name,
      processedAt: r.processedAt,
      shopifyId: r.shopifyId,
      financialStatus: r.financialStatus,
      customerShopifyId: r.customerShopifyId,
    }));
}
...
export async function productMetrics(
  storeId: string,
  period: Period,
  opts: { adSpendMinor?: number; orders?: OrderProfit[] } = {},
): Promise<ProductPeriodMetrics[]> {
  await requireReportPeriod(storeId, period);
  const orders =
    opts.orders ??
    (await orderProfitsInPeriod(storeId, period)).map((o) => o.profit);
  ...
  for (const o of orders) {
    ...
    const orderCosts =
      o.shippingCostMinor +
      o.paymentFeesMinor -
      o.shippingRevenueMinor -
      o.tipsMinor +
      o.dutiesMinor;
    const shares = allocate(
      orderCosts,
      o.lines.map((l) => Math.max(0, l.netSalesMinor)),
    );
    o.lines.forEach((l, i) => {
      ...
      m.missingCogs =
        m.missingCogs || (l.cogsSource === "MISSING" && l.cogsQuantity > 0);
      byProduct.set(key, m);
    });
  }
  ...
  if (opts.adSpendMinor && opts.adSpendMinor > 0 && list.length) {
    const alloc = allocate(
      opts.adSpendMinor,
      list.map((m) => Math.max(0, m.netSalesMinor)),
    );
    list.forEach((m, i) => (m.allocatedAdSpendMinor = alloc[i]));
  }
  for (const m of list)
    m.contributionProfitMinor =
      m.grossProfitMinor - m.allocatedCostsMinor - m.allocatedAdSpendMinor;
