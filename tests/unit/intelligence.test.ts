...
import {
  ...
  summarizePeriod,
} from "../../app/domain/profit/aggregate";
import {
  computeConfidence,
  confidenceLabel,
} from "../../app/domain/confidence";
...
describe("expenseAmountInPeriod", () => {
  it("one-time expense lands in the period containing its date", () => {
    const e = {
      ...
      startsOn: D("2026-05-15T00:00:00Z"),
      endsOn: null,
    };
  ...
  it("accrues monthly expenses per day", () => {
    const e = {
      ...
      startsOn: D("2026-01-01T00:00:00Z"),
      endsOn: null,
    };
  ...
  it("respects start/end bounds", () => {
    const e = {
      ...
      startsOn: D("2026-05-21T00:00:00Z"),
      endsOn: D("2026-05-31T00:00:00Z"),
    };
...
describe("summarizePeriod", () => {
  const orders = [
    computeOrderProfit(order(), ctx()),
    computeOrderProfit(order({ id: "o2", isTest: true }), ctx()),
  ];
    ...
    expenses: [
      {
        ...
        currency: "EUR",
        recurrence: "ONE_TIME",
        startsOn: D("2026-05-02T00:00:00Z"),
        endsOn: null,
      },
    ],
  });
  ...
  it("combineSnapshots reproduces the same totals", () => {
    ...
    expect(combined.contributionProfitMinor).toBe(
      summary.contributionProfitMinor * 2,
    );
...
describe("computeConfidence", () => {
  ...
  it("is low when nothing is connected", () => {
    const r = computeConfidence({
      ...full,
      ordersSynced: false,
      orderSyncAgeMinutes: null,
      refundsSynced: false,
      transactionsSynced: false,
      cogsCoveragePct: 0,
      productsMissingCogs: 50,
      ordersWithoutShippingCost: 100,
      paymentFeeActualPct: 0,
      feeConfigConfirmed: false,
...
function product(
  overrides: Partial<ProductPeriodMetrics> & { productId: string },
  ...
  return {
    ...
    cogsMinor: 4_000,
    grossProfitMinor: 6_000,
    allocatedCostsMinor: 1_000,
    allocatedAdSpendMinor: 1_000,
    contributionProfitMinor: 4_000,
    missingCogs: false,
    refundedUnits: 0,
    ...overrides,
  };
...
describe("detectLeaks", () => {
  ...
  it("is silent when nothing changed", () => {
    expect(
      detectLeaks({ ...base, current: summary({}), previous: summary({}) }),
    ).toEqual([]);
  });
  ...
  it("flags ad spend increases with measured evidence and stable fingerprints", () => {
    const leaks = detectLeaks({
      ...base,
      current: summary({
        adSpendMinor: 260_000,
        contributionProfitMinor: 232_000,
      }),
      previous: summary({}),
    });
    ...
    const again = detectLeaks({
      ...
      previous: summary({}),
    }).find((l) => l.type === "AD_SPEND_INCREASE")!;
  ...
  it("flags margin shrink, refunds, shipping, discounts and fee changes", () => {
    const leaks = detectLeaks({
      ...base,
      current: summary({
        contributionMarginBps: 2400,
        refundRateBps: 950,
        refundsMinor: 90_000,
        shippingCostMinor: 80_000,
        discountRateBps: 1100,
        discountsMinor: 110_000,
        paymentFeesMinor: 40_000,
      }),
      previous: summary({}),
    });
    ...
    const leaks = detectLeaks({
      ...
      products: [
        ...
        product({
          productId: "thin",
          netSalesMinor: 10_000,
          contributionProfitMinor: 500,
        }),
        product({ productId: "nocogs", missingCogs: true, cogsMinor: 0 }),
        product({ productId: "fine" }),
      ],
    });
    expect(leaks.find((l) => l.type === "LOSS_MAKING_PRODUCT")?.entityId).toBe(
      "loss",
    );
    expect(
      leaks.find((l) => l.type === "NEAR_BREAK_EVEN_PRODUCT")?.entityId,
    ).toBe("thin");
  ...
  it("flags stale data and expense spikes", () => {
    const leaks = detectLeaks({
      ...base,
      orderSyncAgeMinutes: 60 * 30,
      current: summary({ otherExpensesMinor: 90_000 }),
      previous: summary({}),
    });
  ...
  it("orders by severity then impact", () => {
    const leaks = detectLeaks({
      ...
      orderSyncAgeMinutes: 60 * 30,
      current: summary({
        adSpendMinor: 400_000,
        contributionProfitMinor: 92_000,
        contributionMarginBps: 989,
      }),
      previous: summary({}),
    });
    const rank = { CRITICAL: 0, HIGH: 1, MEDIUM: 2, LOW: 3 };
    for (let i = 1; i < leaks.length; i++) {
      expect(rank[leaks[i - 1].severity]).toBeLessThanOrEqual(
        rank[leaks[i].severity],
      );
    }
  });
});
  ...
  it("multi-line mixed order", () => {
    const o = order({
      lineItems: [
        ...
        line({ id: "b", variantId: "v2", quantity: 2, unitPriceMinor: 3000 }),
      ],
