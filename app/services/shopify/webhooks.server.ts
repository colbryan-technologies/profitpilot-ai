...
/** Store only the routing identifiers needed by dispatch, never raw customer payloads. */
export function minimalWebhookPayload(
  payload: unknown,
  ...
  const request = p.data_request as { id?: unknown } | undefined;
  const edit = p.order_edit as { order_id?: unknown } | undefined;
  return JSON.parse(
    JSON.stringify({
      id: id(p.id),
      order_id: id(p.order_id),
      processed_at:
        typeof p.processed_at === "string" ? p.processed_at : undefined,
      order_edit: edit ? { order_id: id(edit.order_id) } : undefined,
  ...
  try {
    const event = await prisma.webhookEvent.create({
      data: {
        ...
        payload: minimalWebhookPayload(params.payload),
      },
      select: { id: true },
    });
    ...
    if ((err as { code?: string }).code === "P2002") {
      ...
      if (existing?.status === "RECEIVED") await enqueueWebhook(existing.id);
      logger.info(
        { webhookId: params.webhookId, topic: params.topic },
        "duplicate webhook recovered or already handled",
      );
      return false;
    }
    throw err;
...
async function dispatch(
  ...
  switch (topic) {
    ...
    case "CUSTOMERS_REDACT": {
      ...
      const orders =
        (payload.orders_to_redact as Array<number | string> | undefined) ?? [];
      const count = await redactCustomer(
        storeId,
        customer?.id ? [gid("Customer", customer.id)] : [],
        orders.map((o) => gid("Order", o)),
      );
      await audit({
        storeId,
        actorType: "webhook",
        action: "gdpr.customers_redact",
        metadata: { ordersUpdated: count },
      });
      return "PROCESSED";
    }
  ...
  switch (topic) {
    ...
    case "REFUNDS_CREATE": {
      const rawId =
        topic === "REFUNDS_CREATE" || topic === "ORDERS_EDITED"
          ? ((payload.order_id as number | string | undefined) ??
            (payload.order_edit as { order_id?: number } | undefined)?.order_id)
      ...
      const changed = await syncSingleOrder(
        storeId,
        shopDomain,
        gid("Order", rawId),
      );
    ...
    case "APP_SCOPES_UPDATE": {
      ...
      await prisma.installation.updateMany({
        where: { storeId, uninstalledAt: null },
        data: { grantedScopes: current.join(",") },
      });
      return "PROCESSED";
    }
    case "SHOP_UPDATE": {
    ...
    case "APP_SUBSCRIPTIONS_UPDATE": {
      // Display names are not plan handles. Refresh verified entitlements on
      // the next request instead of trusting payload names for paid access.
