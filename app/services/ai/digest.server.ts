...
import { AiUnavailableError, complete, estimateCostMicros } from "./provider.server";
...
export function deterministicDigest(g: GroundingPack): string {
  ...
  if (g.drivers.length) parts.push(`What moved: ${g.drivers.slice(0, 3).join("; ")}.`);
  if (g.openLeaks.length) parts.push(`Open leaks: ${g.openLeaks.slice(0, 2).map((l) => `${l.title}${l.impact ? ` (${l.impact})` : ""}`).join("; ")}.`);
  parts.push(`Profit Confidence: ${g.confidence.label} (${g.confidence.score}/100).${g.confidence.caveats[0] ? ` ${g.confidence.caveats[0]}` : ""}`);
  return parts.join(" ");
}

export async function generateWeeklyDigest(storeId: string): Promise<{ id: string; summary: string }> {
  ...
  const existing = await prisma.intelligenceDigest.findUnique({ where: { storeId_kind_periodStart: { storeId, kind: "WEEKLY", periodStart: period.start } } });
  ...
  let summary: string;
  try {
    const result = await complete([{ role: "system", content: DIGEST_PROMPT }, { role: "system", content: `DATA:\n${JSON.stringify(data)}` }, { role: "user", content: "Write this week's briefing." }], { tier: "strong", maxOutputTokens: 350 });
    const v = validateAnswer(result.text, grounding);
    summary = v.ok && result.text.trim() ? result.text.trim() : deterministicDigest(grounding);
    await prisma.aiUsage.create({ data: { storeId, feature: "digest", provider: result.provider, model: result.model, inputTokens: result.inputTokens, outputTokens: result.outputTokens, costMicros: estimateCostMicros(result.provider, result.model, result.inputTokens, result.outputTokens), latencyMs: result.latencyMs, success: v.ok } });
    if (!v.ok) logger.warn({ storeId, unverified: v.unverifiedNumbers }, "digest failed validation; used deterministic text");
  } catch (err) {
    if (!(err instanceof AiUnavailableError)) logger.warn({ storeId, err: (err as Error).message }, "digest: provider failed");
    summary = deterministicDigest(grounding);
  }
  const row = await prisma.intelligenceDigest.create({
    data: { storeId, kind: "WEEKLY", periodStart: period.start, periodEnd: period.end, metricsJson: JSON.parse(JSON.stringify({ ...data, previous: previousPeriod(period) })), summary },
  });
...
export async function runDueDigests(now = new Date()): Promise<number> {
  const stores = await prisma.store.findMany({
    where: { status: "ACTIVE", notificationPref: { weeklyDigest: true }, syncJobs: { some: { type: "HISTORICAL_ORDERS", status: "COMPLETED" } } },
    select: { id: true, ianaTimezone: true, notificationPref: { select: { deliveryHour: true } } },
  });
  let n = 0;
  for (const s of stores) {
    const local = new Intl.DateTimeFormat("en-US", { timeZone: s.ianaTimezone, hour: "numeric", hour12: false, weekday: "short" }).formatToParts(now);
    const hour = Number(local.find((p) => p.type === "hour")?.value ?? -1) % 24;
    const weekday = local.find((p) => p.type === "weekday")?.value;
    if (weekday !== "Mon" || hour !== (s.notificationPref?.deliveryHour ?? 8)) continue;
    try {
      await generateWeeklyDigest(s.id);
      n++;
    } catch (err) {
      logger.error({ storeId: s.id, err: (err as Error).message }, "digest generation failed");
    }
  }
  return n;
}
