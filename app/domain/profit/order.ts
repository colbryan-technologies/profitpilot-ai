...
): ShippingRuleInput | null {
  if (rules.length === 0) return null;
  const sorted = [...rules].sort((a, b) => b.priority - a.priority);
  const specific = countryCode
    ? sorted.find(
...
function estimateShippingCost(
  ...
  const cost =
    rule.flatMinor +
    rule.perItemMinor * shippableUnits +
    percentBps(shippingRevenueMinor, rule.percentBps);
...
function computePaymentFees(order: OrderInput, ctx: CalculationContext) {
  ...
  for (const t of charges) {
    if (t.feeMinor !== null) {
      ...
      const override = t.gateway
        ? ctx.fees.gatewayOverrides[t.gateway.toLowerCase()]
        : undefined;
      const cfg = override ?? ctx.fees;
      // Manual / gift-card / free gateways carry no processor fee.
      if (t.gateway && /^(manual|gift_card|bogus|cash|free)/i.test(t.gateway))
        continue;
      estimated += percentBps(t.amountMinor, cfg.percentBps) + cfg.fixedMinor;
    }
  }
  const source =
    actualCount === charges.length
      ? "ACTUAL"
      : actualCount === 0
        ? "ESTIMATED"
        : "MIXED";
  return {
    fees: actual + estimated,
    source: source as "ACTUAL" | "ESTIMATED" | "MIXED",
  };
}
...
export function computeOrderProfit(
  ...
  if (currency !== ctx.currency.toUpperCase())
    throw new Error(
      "Order and calculation currency must match; currency conversion is not configured",
    );
  const excludedReason = order.isTest
    ? "test order"
    : order.cancelledAt &&
        order.transactions.every((t) => !isChargeTransaction(t))
      ? "cancelled before payment"
      : null;

  const refundedByLine = new Map<
    string,
    { qty: number; restockedQty: number; subtotal: number; tax: number }
  >();
  ...
  for (const r of order.refunds) {
    ...
    for (const rl of r.lineItems) {
      const cur = refundedByLine.get(rl.lineItemId) ?? {
        qty: 0,
        restockedQty: 0,
        subtotal: 0,
        tax: 0,
  ...
  for (const li of order.lineItems) {
    ...
    const refunded = refundedByLine.get(li.id) ?? {
      qty: 0,
      restockedQty: 0,
      subtotal: 0,
      tax: 0,
    ...
    const taxAdjustment = order.taxesIncluded
      ...
      : ctx.taxTreatment === "INCLUDE_TAX_AS_REVENUE"
        ...
        : 0;
    const refundedPrice =
      refunded.subtotal + (order.taxesIncluded ? refunded.tax : 0);
    const netSales = gross - li.discountMinor - refundedPrice + taxAdjustment;
    ...
    // Returning delivered goods does not refund the carrier charge.
    if (li.requiresShipping) shippableUnits += li.quantity;
  ...
  const shippingTax = Math.max(
    0,
    order.totalTaxMinor - order.lineItems.reduce((a, l) => a + l.taxMinor, 0),
  );
  ...
  const contribution = excludedReason
    ? 0
    : grossProfit +
      shippingRevenue -
      shipping.cost -
      fees.fees +
      order.totalTipMinor -
      order.totalDutiesMinor;
  const coverage =
    cogsQtyTotal === 0 ? 100 : Math.round((cogsQtyKnown / cogsQtyTotal) * 100);
