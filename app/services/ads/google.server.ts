...
async function call<T>(
  ...
  if (!res.ok) {
    ...
    throw new AdsProviderError(
      `Google Ads API error ${res.status}: ${msg}`,
...
export function microsToMinor(
  ...
  return toMinor(new Decimal(micros).div(1_000_000).toFixed(), currency);
}
...
export const googleAds: AdsProvider = {
  ...
  async exchangeCode(code, redirectUri) {
    ...
    const t = await call("https://oauth2.googleapis.com/token", tokenSchema, {
      method: "POST",
      body,
  ...
  async refresh(creds) {
    if (!creds.refreshToken)
      throw new AdsProviderError("No refresh token", false, true);
    const body = new URLSearchParams({
  ...
  async fetchDailySpend(creds, externalId, start, end) {
    ...
    do {
      const r = await call(
        `${ADS_API}/customers/${externalId}/googleAds:search`,
        metricsSchema,
        {
          method: "POST",
      ...
      for (const row of r.results) {
        ...
        out.push({
          ...
          attributedRevenueMinor:
            row.metrics.conversionsValue !== undefined
              ? microsToMinor(
                  Math.round(row.metrics.conversionsValue * 1_000_000),
                  currency,
                )
              : null,
          attributedConversions:
            row.metrics.conversions !== undefined
              ? Math.round(row.metrics.conversions)
              : null,
        });
      }
